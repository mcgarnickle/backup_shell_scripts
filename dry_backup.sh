...
if [[ ! -z $3 ]]; then
  #sublime text
  rsync -navh ~/.config/sublime-text-3/Packages/User/Preferences.sublime-settings "$srcDir"config/sublime-text/ | grep -v 'uptodate'
  #squeezebox
  rsync -navh /var/lib/squeezeboxserver/prefs/ "$srcDir"config/squeezebox/ | grep -v 'uptodate'
  #firefox
  rsync -navh /home/alan/.mozilla/firefox/wmc89qgm.default/sessionstore.* "$srcDir"config/firefox/ | grep -v 'uptodate'
  rsync -navh /home/alan/.mozilla/firefox/wmc89qgm.default/sessionbackups/ "$srcDir"config/firefox/sessionbackups/ | grep -v 'uptodate'
  rsync -navh /home/alan/.mozilla/firefox/wmc89qgm.default/sessions/ "$srcDir"config/firefox/sessions/ | grep -v 'uptodate'
