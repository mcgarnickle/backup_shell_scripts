...
if [[ ! -z $3 ]]; then
  #sublime text
  sudo rsync -avh ~/.config/sublime-text-3/Packages/User/Preferences.sublime-settings "$srcDir"config/sublime-text/ | grep -v 'uptodate'
  #squeezebox
  sudo rsync -avh /var/lib/squeezeboxserver/prefs/ "$srcDir"config/squeezebox/ | grep -v 'uptodate'
  #firefox
  sudo rsync -avh /home/alan/.mozilla/firefox/wmc89qgm.default/sessionstore.* "$srcDir"config/firefox/ | grep -v 'uptodate'
  sudo rsync -avh /home/alan/.mozilla/firefox/wmc89qgm.default/sessionbackups/ "$srcDir"config/firefox/ | grep -v 'uptodate'
  sudo rsync -avh /home/alan/.mozilla/firefox/wmc89qgm.default/sessions/ "$srcDir"config/firefox/sessions/ | grep -v 'uptodate'
